...
if __name__ == '__main__':
    dataset = 'twosides'
    logs = log_util(dataset,'0.7')
    for fold in [1]:

        train_dir = './txt/train{}.txt'.format(fold)
        test_dir = './txt/test{}.txt'.format(fold)
        train_data = prepare_twosides(train_dir)
        test_data = prepare_twosides(test_dir)

        if torch.cuda.is_available():

            model = DD_Pre(45,0.7,0.7).cuda()

        train_loader = twosides_pkl_loader(train_data,batch_size=2048,shuffle=True,num_workers=2,pin_memory = True)
        test_loader = twosides_pkl_loader(test_data,batch_size=1024,shuffle=False,num_workers=0,pin_memory = True)
